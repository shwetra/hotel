...
import { HotelData, HotelDelete } from "../Redux/AppReducer/action";
...
const HotelList = () => {
  ...
  const us=useSelector((store)=>store.AppReducer.hotel)
  useEffect(()=>{
  dispatch(HotelData)
  },[])

  const handleDelete = (_id) => {
    dispatch(HotelDelete(_id)).then((res) => {
      if (res.type === "DELETE_HOTEL_SUCCESS") {
        dispatch(HotelData);
      } else {
      ...
      <Box overflowX="auto" w={{ base: "100%", md: "100%", xl: "100%" }} border="3px solid black" borderRadius="15px">
        <Table  size="md" variant="simple">
          <Thead bgColor="teal.100">
            <Tr>
              <Th w="50%" color={"Black"} fontWeight={"bold"}>Hotel Name</Th>
          ...
          <Tbody bgColor="gray.100">
            {us && us.map((e) => (
              <Tr key={e._id} border="2px solid black">
                <Box as="Td" color={"Black"} textAlign={"left"} fontWeight={"500"} w="80%" overflow="hidden" textOverflow="ellipsis" whiteSpace="nowrap">{e.Name}</Box>
                  <Td>
